...
#include "framework.h"
#include "PPE.h" //conține declarații pentru toate funcțiile din API-ul 
//Windows, toate macrocomenzile comune utilizate in windows

#define MAX_LOADSTRING 100
HINSTANCE hInst; // HINSTANȚĂ este un „mâner pentru o instanță”. 

// Transmite declarații de funcții incluse în acest modul de cod:
WCHAR szWindowClass[MAX_LOADSTRING];  // numele clasei ferestrei principale
...
HWND mHwnd;
RECT clientRect; //Rect reprezintă un dreptunghi abstract și poate fi folosit într-o varietate de situații
    ...
    _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_PPE, szWindowClass, MAX_LOADSTRING);// Inițializați șirurile globale
    MyRegisterClass(hInstance);

    if (!InitInstance(hInstance, nCmdShow))// Efectuați inițializarea aplicației:
    {
        return FALSE;
    }
...
ATOM MyRegisterClass(HINSTANCE hInstance) //Înregistrează clasa ferestrei.
{
    WNDCLASSEXW wcex;
...
// În această funcție, salvăm mânerul instanței într-o variabilă globală și
// creează și afișează fereastra principală a programului.
{
    hInst = hInstance; // Stochează mânerul instanței în variabila noastră globală
    HWND hWnd = CreateWindowW(szWindowClass, L" Lab 4-5 gg TI-", WS_MINIMIZEBOX | WS_SYSMENU,
...
//  Procesează mesajele pentru fereastra principală.
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE: {
        ...
        textA = CreateWindow(L"static", L"2+2^2=:", WS_CHILD | WS_BORDER, 140, 128, 70, 20, hWnd, 0, hInst, NULL);
        textP = CreateWindow(L"static", L"3^2=:", WS_CHILD | WS_BORDER, 140, 201, 70, 20, hWnd, 0, hInst, NULL);
        ...
        radiob1 = CreateWindow(L"button", L"FUA",
            WS_CHILD | BS_AUTORADIOBUTTON,
            40, 162, 54, 33, hWnd, 0, hInst, NULL);
        radiob2 = CreateWindow(L"button", L"FEIE",
            WS_CHILD | BS_AUTORADIOBUTTON,
            223, 162, 54, 33, hWnd, 0, hInst, NULL);
        radiob3 = CreateWindow(L"button", L"FCIM",
            WS_CHILD | BS_AUTORADIOBUTTON,
            422, 162, 54, 33, hWnd, (HMENU)IDC_RADIOBUT, hInst, NULL);

        check1 = CreateWindow(L"button", L"Nr. 12",
            WS_CHILD | BS_AUTOCHECKBOX,
            39, 126, 83, 33, hWnd, 0, hInst, NULL);
        check2 = CreateWindow(L"button", L"Nr. 1",
            WS_CHILD | BS_AUTOCHECKBOX,
            223, 126, 111, 33, hWnd, (HMENU)IDC_CHECKBOX_CTRL_ID, hInst, NULL);
        check3 = CreateWindow(L"button", L"Nr. 10",
            WS_CHILD | BS_AUTOCHECKBOX,
            422, 126, 71, 33, hWnd, 0, hInst, NULL);
        check4 = CreateWindow(L"button", L"Nr. 2",
            WS_CHILD | BS_AUTOCHECKBOX,
            39, 198, 82, 33, hWnd, (HMENU)IDC_CHECKBOX_CTRL_ID2, hInst, NULL);
        check5 = CreateWindow(L"button", L"Nr. 11",
            WS_CHILD | BS_AUTOCHECKBOX,
            223, 198, 95, 33, hWnd, 0, hInst, NULL);
        check6 = CreateWindow(L"button", L"Nr. 13",
            WS_CHILD | BS_AUTOCHECKBOX,
            422, 198, 86, 33, hWnd, 0, hInst, NULL);

        text1 = CreateWindow(L"static", L"Cum se numeste facultatea ", WS_CHILD | WS_BORDER, 151, 66, 233, 20, hWnd, 0, hInst, NULL);
        text2 = CreateWindow(L"static", L"Care camine au 12 etaje?:", WS_CHILD | WS_BORDER, 100, 66, 200, 20, hWnd, 0, hInst, NULL);
        text3 = CreateWindow(L"static", L"Calculati:", WS_CHILD | WS_BORDER, 151, 66, 160, 20, hWnd, 0, hInst, NULL);
    ...
    case WM_COMMAND:  //procesează meniul aplicației
    ...
    case WM_PAINT:  //Pictează fereastra principală
    {
        PAINTSTRUCT ps;
    ...
    case WM_DESTROY: // postați un mesaj de renunțare și reveniți
        PostQuitMessage(0);
        break;
    default:
